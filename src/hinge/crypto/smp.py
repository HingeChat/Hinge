...
def packList(*items):
    ...
    for item in items:
        ...
        buffer += struct.pack('!I', len(new_bytes))
        buffer += new_bytes
    return buffer
...
def bytesToLong(in_bytes):
    length = len(in_bytes)
    string = 0
    for i in range(length):
        string += byteToLong(in_bytes[i:i+1]) << 8 * (length - i - 1)
    return string
...
def longToBytes(long):
    new_bytes = b''
    while long != 0:
        new_bytes = longToByte(long & 0xff)
        new_bytes += bytes
        long >>= 8
    return new_bytes
...
def createRandomExponent():
    ...
    new_dec = CryptoUtils.binToDec(new_bytes)
    return new_dec
